...
const Events = () => {
  ...
  return (
    <div>
      <table className='table-auto border-4 border-r-4'>
        <thead>
          <tr>
            <th className='p-3 w-fit border-r-4 text-sm'>Event Name</th>
            <th className='p-3 w-fit border-r-4 text-sm'>Event Venue</th>
            <th className='p-3 w-fit border-r-4 text-sm'>Start Time</th>
            <th className='p-3 w-fit border-r-4 text-sm'>Date</th>
            <th className='p-3 w-fit border-r-4 text-sm'>
              Registration Fee
              <button onClick={toggleSortOrder}>
                {sortOrder === 'asc' ? '▲' : '▼'}
              </button>
            </th>
            <th className='p-3 w-fit border-r-4 text-sm'>Event Detail</th>
            <th className='p-3 w-fit border-r-4 text-sm'>Organizer's Name</th>
            <th className='p-3 w-fit border-r-4 text-sm'>Registration Link</th>
          </tr>
        </thead>
        <tbody>
          {sortedEvents.map((event, index) => (
            <tr key={index} className='hover:font-bold transition-all'>
              <td className='p-3 w-fit border-4 ease-in  font-light text-sm'>{event.name}</td>
              <td className='p-3 w-fit border-4 ease-in  font-light text-sm'>{event.venue}</td>
              <td className='p-3 w-fit border-4 ease-in  font-light text-sm'>{event.time}</td>
              <td className='p-3 w-fit border-4 ease-in  font-light text-sm'>{event.date}</td>
              <td className='p-3 w-fit border-4 ease-in  font-light text-sm'>{event.registration_fee}</td>
              <td className='p-3 w-fit border-4 ease-in  font-light text-sm'>{event.description}</td>
              <td className='p-3 w-fit border-4 ease-in  font-light text-sm'>{event.organiser_name}</td>
              <td className='p-3 w-fit border-4 ease-in  font-light text-sm'>
